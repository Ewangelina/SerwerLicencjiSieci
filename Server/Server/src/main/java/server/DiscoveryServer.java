...
public class DiscoveryServer implements Runnable {
	...
	public DiscoveryServer(String finalPort)
	{
		this.finalPort = finalPort;
	}
	public static void main(String[] args) {
		DiscoveryServer.startListening("7777");
	}

	public static void startListening(String finalPort)
	{
		DiscoveryServer server = new DiscoveryServer(finalPort);
		server.run();
	}
...
//			InetAddress addr = NetworkUtil.getMyAddress();
		...
		while (true) {
			...
			if (message.startsWith(DISCOVERY_REQUEST)) {
				String reply =  DISCOVERY_REPLY + MY_IP + ":" + finalPort;
