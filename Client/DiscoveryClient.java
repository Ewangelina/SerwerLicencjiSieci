	...
	public String call() {
		...
		while(true) {
			try {
				...
				// Does is match?
				int k = reply.indexOf(DiscoveryClient.DISCOVERY_REPLY);
				if (k<0) {
					logger.warning("Reply does not contain prefix "+DiscoveryClient.DISCOVERY_REPLY);
					break;
				}
				k += DiscoveryClient.DISCOVERY_REPLY.length(); // skip prefix
